...
const checkExtension = () => {
    let extensionItem = $('.wrap-item__value__extension-request').length - 1;
...
//check Option when reload page.
const checkOption = () => {
    let item = $('.wrap-item__value__option-request').length - 1;
    if (item > TOTAL_OPTION) {
        let appendAddExtension = `
        <img src="/assets/img/clients/teacher/add-time.svg" alt="">
        オプションを追加する（あと${QUANTITY_ADD_OPTION - item - 1}件)`;
        $('.add-option').empty();
        $('.add-option').append(appendAddExtension);
    }
    if (item + 1 >= QUANTITY_ADD_OPTION) {
        $('.add-option').hide();
    }
}
$('body').on('click', '.add-extension', function () {
    ...
    let extensionItem = $('#list-extension-request .wrap-item__value__extension-request').length - 1;
    TOTAL_EXTENSION >= 2 || extensionItem >= 2 ? $('.add-extension').hide() : '';
