...
<div class="course-end-email">
    ...
    <div class="course-end-email__text-note">
        <p style="margin: 0;">下記のサービスについて、出品者の{{$teacherFullName}}さんから連絡がありました。</p>
        <hr>
    </div>
    ...
    @if($enabledRequestRestock == 'false')
        <div class="course-end-email__course-info">
            <div class="fit-content">
                <p style="margin: 0;" class="course-end-email__course-info__title">サービスの詳細</p>
                <hr>
            </div>
            <p style="margin: 0;" class="course-end-email__course-info__item">
                サービス：{{$courseSchedule['title']}}
            </p>
            <p style="margin: 0;" class="course-end-email__course-info__item">
                開催日：{{now()->parse($courseSchedule['start_datetime'])->format('m月d日')}}
            </p>
            <p style="margin: 0;" class="course-end-email__course-info__item">
            ...
            <p style="margin: 0;" class="course-end-email__course-info__item">
                金額 ： {{number_format($courseSchedule['price'])}}円
            </p>
            <hr>
        </div>
    @endif
    <div class="course-end-email__course-info">
        ...
        <p style="margin: 0;" class="course-end-email__course-info__item">
            @if($checkPurchased === 0)
                <a href="{{route('client.student.message.list', 'sort=1&perPage=10&page=1&option=4')}}">{{route('client.student.message.list', 'sort=1&perPage=10&page=1&option=4')}}</a>
            @else
                <a href="{{route('client.student.message.list', 'sort=1&perPage=10&page=1&option=1')}}">{{route('client.student.message.list', 'sort=1&perPage=10&page=1&option=1')}}</a>
            @endif
        </p>
        <hr>
    </div>
    ...
    <div class="course-end-email__content">
        <div class="course-end-email__content__note" style="display: flex;">
            <p style="margin: 0;">※</p>
