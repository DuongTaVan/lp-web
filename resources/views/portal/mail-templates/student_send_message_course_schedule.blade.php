...
<div class="course-end-email">
    ...
    <div class="course-end-email__text-note">
        <p style="margin: 0;">下記のサービスについて、@if($checkPurchased != 0)購入者の@endif {{$studentFullName}} @if($checkPurchased === 0)
            さんから質問がありました。@else
            さんからメッセージがありました。@endif</p>
        <hr>
    </div>
    ...
    <div class="course-end-email__course-info">
        ...
        <div class="course-end-email__course-info__item">
            ...
            @else
                <a href="{{route('client.teacher.my-page.message.message-course')}}">{{route('client.teacher.my-page.message.message-course')}}</a>
            @endif
        </div>
        <hr>
    </div>
