                    ...
                    <div class="explain-content">
                        ...
                        <div class="explain-content__option-btn position-relative">
                            ...
                            @if($course->approval_status == \App\Enums\DBConstant::COURSE_NOT_REVIEW)
                                @if(auth('client')->user()->user_status === \App\Enums\Constant::USER_STATUS_REST)
                                    @if (auth('client')->user()->identity_verification_status != \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED)
                                        <a class="explain-content__option-btn__publish {{ auth('client')->user()->identity_verification_status != \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED ? 'disabled-public' : ''}}">承認申請</a>
                                    @else
                            ...
                            @else
                                @if(auth('client')->user()->user_status === \App\Enums\Constant::USER_STATUS_REST)
                                    @if (auth('client')->user()->identity_verification_status != \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED)
                                        <a class="explain-content__option-btn__publish {{ auth('client')->user()->identity_verification_status != \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED ? 'disabled-public' : ''}}">公開する</a>
                                    @else
