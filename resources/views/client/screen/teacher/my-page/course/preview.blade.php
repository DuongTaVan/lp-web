...
@section('content')
                ...
                <div class="left content-description">
                    ...
                    <div class="explain-content">
                        ...
                        @if(auth()->guard('client')->user()->teacher_category_skills != \App\Enums\DBConstant::TEACHER_CATEGORY_SKILLS)
                            @php
                                $extensions = (!empty($isSchedule) && $isSchedule && $course->course->extensionsOpen->count()) ? $course->course->extensionsOpen : (isset($course->extensionsOpen) && $course->extensionsOpen->count() > 0 ? $course->extensionsOpen : [])
                            @endphp
                            @if(count($extensions))
                                <section class="sub-course">
                                    <label for="" class="label-option f-w6">※オプション設定</label>
                        ...
                        <div class="explain-content__option-btn position-relative">
                            @php
                                $urlBack = $isSchedule ?
                                    route('client.teacher.course_schedules.show', [$course->course_schedule_id, 'isPublic' => Request::get('isPublic'), 'group' => $group]) :
                                    route('client.teacher.courses.show', ['course' => $course->course_id, 'group' => $group]);

                                if (!$isSchedule && !$course->courseSchedules->count()) {
                                    $urlBack = route('client.teacher.courses.show', ['course' => $course->course_id, 'backFrom' => 'preview', 'isPublic' => Request::get('isPublic')]);
                                }
                                $urlPublic = $isSchedule ? route('client.teacher.course_schedules.public', ['course_schedule_id' => $course->course_schedule_id, 'status' => \App\Enums\DBConstant::COURSE_STATUS_OPEN, 'group' => $group, 'isPublic' => (int)Request::get('isPublic')]) :
                                    route('client.teacher.courses.public', ['courseId' => $course->course_id, 'status' => \App\Enums\DBConstant::COURSE_STATUS_OPEN, 'group' => $group]);
                                $urlDraft = $isSchedule ? route('client.teacher.course_schedules.public', ['course_schedule_id' => $course->course_schedule_id, 'status' => \App\Enums\DBConstant::COURSE_STATUS_DRAFT, 'group' => $group, 'isPublic' => (int)Request::get('isPublic')]) :
                                    route('client.teacher.courses.public', ['courseId' => $course->course_id, 'status' => \App\Enums\DBConstant::COURSE_STATUS_DRAFT, 'group' => $group]);
                            @endphp
                            <a href="{{ $urlBack }}" class="btn-back position-absolute">戻る</a>
                            <a href="{{ $urlBack }}" class="btn-icon-back position-relative">
                                <img src="{{ asset('assets/img/clients/teacher/arrow-left-black.svg') }}" alt="">
                            </a>
                            @if((int)Request::get('isPublic') != 1)
                                <a href="{{ $urlDraft }}"
                            ...
                            @if($course->approval_status === \App\Enums\DBConstant::COURSE_NOT_REVIEW)
                                @if($user->user_status === \App\Enums\Constant::USER_STATUS_REST)
                                    @if ($user->identity_verification_status !== \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED)
                                        <a class="explain-content__option-btn__publish {{ $user->identity_verification_status !== \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED ? 'disabled-public' : ''}}">承認申請</a>
                                    @else
                                        <a href="{{ $urlPublic }}"
                                           class="explain-content__option-btn__publish">承認申請</a>
                                    @endif
                                @endif
                            @else
                                @if($user->user_status === \App\Enums\Constant::USER_STATUS_REST)
                                    @if ($user->identity_verification_status !== \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED)
                                        <a class="explain-content__option-btn__publish {{ $user->identity_verification_status !== \App\Enums\DBConstant::IDENTITY_VERIFICATION_STATUS_APPROVED ? 'disabled-public' : ''}}">公開する</a>
                                    @else
                                        <a href="{{ $urlPublic }}"
                ...
                <div class="right content-table">
                    ...
                    <div class="seller-profile">
                        ...
                        @else
                            <div class="d-flex flex-wrap justify-content-between pd-pc-12">
                                ...
                                @if($user->teacher_category_consultation === \App\Enums\DBConstant::TEACHER_CATEGORY_CONSULTATION
                                    && $user->business_card_verification_status === \App\Enums\DBConstant::BUSINESS_CARD_VERIFICATION_STATUS_APPROVED)
                                    <div
                                        class="nondisclosure-agreement d-flex justify-content-center align-items-center length-div  position-relative @if( isset($course->course) ? $course->course->user->teacher_category_consultation : $course->user->teacher_category_consultation === \App\Enums\DBConstant::TEACHER_CATEGORY_CONSULTATION ) nondisclosure-agreement-small @endif">
                                        <img class="position-absolute icon-nondisclosure-agreement"
                                             src="{{asset('assets/img/clients/course-detail/qualification.svg')}}"
                                             alt="">
                                        <button
                        ...
                        <div class="seller-profile__footer position-relative">
                            @if (isset($user->catchphrase))
                                <p class="seller-profile__footer__catchphrase text-more">{{ $user->catchphrase }}</p>
                                <div class="position-absolute see-all">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            @foreach($course->courseSchedules as $key => $item)
                                <div id="option{{$key}}" data-id="{{ $item["course_schedule_id"] }}"
                                    ...
                                    <img class="icon-arrow position-absolute"
                                         @if($course->courseSchedules->count() > 0) style="display: unset!important;"
                                         @endif
                                         src="{{asset('assets/img/clients/course-detail/arrow-down.png')}}">
                                    <span
    ...
    @include('client.screen.course-detail.choose-schedule-sp', ['data' => $isSchedule ? [$course] : $course->courseSchedules])
