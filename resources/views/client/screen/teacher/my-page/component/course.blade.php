<div id="service" class="tab-pane service-tab fade">
    <div class="service-custom">
        <div class="row service-content" style="background: #fafafa">
            @if(!empty($data['courses']) && $data['courses'] != [])
                <div class="service-teacher-detail" id="service-detail"
                     data-url="{{route('client.teacher.course-detail', $data['userId'])}}">
                        ...
                        @if(count($course->courseSchedules)>0)
                            <div class="d-flex justify-content-center service-detail card__content_detail_info card__content__info__tt__date">
                                ...
                                @php
                                    ...
                                    $bgColor = '';
                                    $canRestock = false;
                                    $userPurchase = count($course->courseSchedules[0]['purchases']) ? $course->courseSchedules[0]['purchases'][0]['user_id'] : null;
                                ...
                                <div class="col-md-10 align-items-center">
                                    ...
                                    @if(count($course->courseSchedules)>0)
                                        <div class="service-public-date">
                                            <div class="row">
                                                <div class="d-flex align-items-center col-md-4 public_date pr-0">
                                                    <svg class="datetime" width="12" height="14" viewBox="0 0 18 20"
                                                         fill="none"
                                                    ...
                                                    <span class="datetime">
                                                        ...
                                                        <div class="card__content__info__tt__date__click content_date"
                                                            ...
                                                            @foreach($course->courseSchedulesCanPurchase as $courseSchedule)
                                                                @if (!$courseSchedule->purchase_id)
                                                                    <div class="card__content__info__tt__date__click__1 content_date_click"
                                                                         ...
                                                                         data-id="{{$courseSchedule['course_schedule_id']}}">
                                                                            <span class="date f-w3">{{Carbon\Carbon::parse($courseSchedule->start_datetime)->format('m')}}<small
                                                                                        class="">月</small>{{Carbon\Carbon::parse($courseSchedule->end_datetime)->format('d')}}<small>日</small></span>
                                        ...
                                        <div class="hozital-line"></div>
                                    @endif
                                    <div class="service-paragraph service-paragraph-{{ $course->course_id }} mt-1">
                    ...
                    @empty
                        <div class="d-flex justify-content-center" style="background: #fafafa">
                            ...
                            <p class="text-center course-no-data" style="background: #fafafa; margin-left: 16.42px"
                               style="padding-bottom: 0">@lang('labels.common.course_no_data')</p>
                        </div>
    ...
    <div class="load-more-link row text-center" id="load" data-pageCourse="1" style="width: 100%">
        <div class="col-md-12 div-load">
            @if(!empty($data['courses']))
                @if($data['courses']->currentPage() < $data['courses']->lastPage())
                    <a href="#" class="see_more_course"
                       data-lastPage= {{$data['courses']->lastPage()}}>{{__('labels.users.teacher_screen.see_more')}}</a>
                @endif
            @endif
        </div>
    </div>
