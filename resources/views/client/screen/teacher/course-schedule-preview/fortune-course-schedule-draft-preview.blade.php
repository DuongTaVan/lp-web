...
{{--                        @if(count($oldImg) > 0)--}}
{{--                            <div>--}}
{{--                                <img class="family" src='{{Storage::disk('s3')->url($oldImg[0])}}' alt="" width="900"--}}
{{--                                     height="497">--}}
{{--                            </div>--}}
{{--                            <div class="three-image d-flex w-100">--}}
{{--                                @if(count($oldImg) >= 2)--}}
{{--                                    @for($i = 1; $i<count($oldImg); $i++)--}}
{{--                                        <img src='{{ Storage::disk('s3')->url($oldImg[$i]) }}' alt="">--}}
{{--                                    @endfor--}}
{{--                                @endif--}}
...
{{--                        @elseif(count($previews) > 0)--}}
{{--                            <div>--}}
{{--                                <img class="family" src='{{$previews[0]}}' alt="" width="900"--}}
{{--                                     height="497">--}}
{{--                            </div>--}}
{{--                            <div class="three-image d-flex w-100">--}}
{{--                                @if(count($previews) >= 2)--}}
{{--                                    @for($i = 1; $i<count($previews); $i++)--}}
{{--                                        <img src='{{$previews[$i]}}' alt="">--}}
{{--                                    @endfor--}}
{{--                                @endif--}}
...
{{--                            <div>--}}
{{--                                <img class="family"--}}
{{--                                     src='{{!empty($course->imagePaths) && count($course->imagePaths) ? $course->imagePaths['0']->image_url : $coursesTemp->imagePaths['0']->image_url }}'--}}
{{--                                     alt="" width="900" height="497">--}}
{{--                            </div>--}}
{{--                            <div class="three-image d-flex w-100">--}}
{{--                                @if(count($course->imagePaths) >= 2)--}}
...
{{--                        @endif--}}
                                        ...
                                        @foreach($time as $key => $t)
                                            <div class="sub-course-time-price">
                                                <div class="price">
                                                    ...
                                                    <span class="usage-time__value">{{$t}}分</span>
                                                </div>
                                                <div class="date-time">
                                                    <div class="usage-time">
                                                        <span class="usage-time__label f-w6">金額</span>
                                                        <span class="price__value">¥{{number_format($money[$key])}}</span>
                                                    </div>

                                                </div>
                                            </div>
                                            @if(count($time) > 1 && $key+1 < count($time))
                        ...
                        <div class="explain-content__option-btn">
                            ...
                            @if(auth('client')->user()->user_status === \App\Enums\Constant::USER_STATUS_REST)
                                <form method="POST"
                                      action="{{route('client.teacher.courses.update.public-course-schedule-draft.fortunetelling', $courseSchedule->course_schedule_id)}}">
                                    @csrf
                                    ...
                                    <input type="hidden" class="f-w3 input-small" placeholder=""
                                           value="{{$course->subtitle}}" name="subtitle">
                                    <input type="hidden" class="f-w3 input-small" placeholder=""
                                           value="{{$course->body}}" name="body">
                                    <input type="hidden" class="f-w3 input-small" placeholder=""
                                           value="{{$course->flow}}" name="flow">
                                    <input type="hidden" class="f-w3 input-small" placeholder=""
                                           value="{{$course->cautions}}" name="cautions">
                                    <input type="hidden" class="f-w3 input-small" placeholder=""
                                           value="{{$course->is_mask_required}}" name="is_mask_required">
{{--                                    @foreach($previews as $preview)--}}
{{--                                        <input type="hidden" name="previews[]" value="{{$preview}}">--}}
{{--                                    @endforeach--}}
{{--                                    @foreach($oldImg as $oI)--}}
{{--                                        <input type="hidden" name="oldImg[]" value="{{$oI}}">--}}
{{--                                    @endforeach--}}
                                    @if(session()->has('preview_file_' . auth('client')->id()))
                                    ...
                                    @endif
                                    @foreach($money as $m)
                                        <input type="hidden" name="sub_course_money[]" value="{{$m}}">
                                    @endforeach
                                    @foreach($time as $t)
                                        <input type="hidden" name="sub_course_time[]" value="{{$t}}">
                                    @endforeach
                                    @foreach($extraTitle as $eT)
                                        <input type="hidden" name="extra_title[]" value="{{$eT}}">
                                    @endforeach
                                    @foreach($extraPrice as $eP)
                                        <input type="hidden" name="extra_price[]" value="{{$eP}}">
                                    @endforeach
{{--                                    <input type="hidden" name="imageId" value="{{$imageId}}">--}}
                                    ...
                                    <button type="submit" class="explain-content__option-btn__publish disabled-public">公開する</button>
                                    @else
                            ...
                            @if(count($extraTitle) > 0)
                                <section id="charged-option">
                                    ...
                                    <div class="table-option disable-option">
                                        @foreach($extraTitle as $key => $item)
                                            <div class="table-option-item d-flex align-items-center justify-content-between">
                                                ...
                                                <div>
                                                    <img src="{{asset('assets/img/clients/course-detail/add.svg')}}"
                                                         alt="">
                                                    <span title="¥{{number_format($extraPrice[$key])}}">¥{{number_format($extraPrice[$key])}}</span>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
    ...
    <div class="Admission-fee-app">
        @if(!empty($course->category->type))
            ...
            @endswitch
        @endif
            <span class="mr-5">鑑定料</span>
        <span class="text-bold"></span>
        <small>{{number_format($courseSchedule['price'])}}(@lang('labels.course-detail.tax_included_course'))</small>
                        ...
                        <div
                            ...
                            <div style="white-space: nowrap">
                                <img
                                        ...
                                        alt="">
                                <span
                                        class="option-price">¥{{number_format($extraPrice[$key])}}</span>
                            </div>
                        </div>
                    @endforeach
                @endif
