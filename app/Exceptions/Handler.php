...
//            return $this->toResponse($request, ErrorType::CODE_5001, __('errors.MSG_5001'), ErrorType::STATUS_5001);
            return $this->toResponse($request, ErrorType::CODE_5001, $exception->getMessage(), ErrorType::STATUS_5001);
        }

        if ($exception instanceof MaintenanceModeException) {
            return response()->view('common.503');
        }

        // HTTP Exception
        if ($exception instanceof ModelNotFoundException) {
            return response()->view('common.404');
        }
        ...
        if ($this->isHttpException($exception)) {
            switch ($exception->getStatusCode()) {
                case ErrorType::STATUS_4030:
                    return response()->view('common.403');
                case ErrorType::STATUS_4190:
                    return response()->view('common.419');
                default:
                    return response()->view('common.404');
            }
        }
        ...
        if ($exception->getPrevious()) {
            if (get_class($exception->getPrevious()) === "Illuminate\Database\Eloquent\ModelNotFoundException") {
                return response()->view('common.404');
            }
        }
